...
class PerformancesBarChartPage extends React.Component {
	...
	setupChartData() {
		...
		const scenariosValues = performancesModel.scenarios.map(
			(scenario, index) => ({
				name: scenario.name,
				file: scenario.file,
				x: `${+(index + 1)}`,
	...
	_getScenariosTooltip(x) {
		const currentScenario = this.chartData.scenariosChartData[0].values.find(
			scenario => scenario.x === x
		);
		return (
			<div className="wrapper-scenario-chart-tooltip">
				...
				<h5 className="single-scenario-duration">
					{currentScenario.y.toString()}s
				</h5>
			</div>
		);
	}
	...
	_getStepsTooltip(x) {
		const currentStep = this.chartData.stepsChartData[0].values.find(
			step => step.x === x
		);
		return (
			<div className="wrapper-scenario-chart-tooltip">
	...
	render() {
		return (
			<div>
				<DocumentTitle title="Performances Bar Chart" />
				<PerformancesBarChart
					tooltip={this._getScenariosTooltip}
					data={this.chartData.scenariosChartData}
				/>
				<PerformancesBarChart
					tooltip={this._getStepsTooltip}
					data={this.chartData.stepsChartData}
				/>
			</div>
		);
	}
